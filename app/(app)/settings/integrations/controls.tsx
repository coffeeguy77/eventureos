...
export function DisconnectButton({ provider, name }: { provider: string; name: string }) {
  ...
  return (
    <>
      <Button size="sm" variant="danger" disabled={pending} className="h-10 sm:h-8"
        onClick={() => {
...
export function ImportForm({ months, inProgress }: { months: number; inProgress: boolean }) {
  ...
  return (
    <form action={action} className="flex flex-wrap items-end gap-3 px-4 pb-5 sm:px-5">
      <div className="w-full sm:w-auto">
        <Label htmlFor="months">Look back</Label>
        <Select id="months" name="months" defaultValue={String(months)} className="w-full sm:w-40">
...
export function XeroSettingsForm({ values, tenants }: {
  ...
  return (
    <form action={action} className="grid gap-4 px-4 pb-5 sm:grid-cols-2 sm:px-5">
      {tenants.length > 1 && (
        <div className="sm:col-span-2">
    ...
    <div className="space-y-2">
      <div className="flex flex-wrap items-center gap-2">
        <form action={merge} className="flex w-full flex-wrap items-center gap-2 sm:w-auto">
          ...
          <Button size="sm" variant="primary" disabled={busy} className="h-10 sm:h-8">{mPending ? "Merging…" : mergeLabel}</Button>
        </form>
        <form action={keep}><Button size="sm" variant="secondary" disabled={busy} className="h-10 sm:h-8">{kPending ? "Saving…" : separateLabel}</Button></form>
        <form action={ignore}><Button size="sm" variant="ghost" disabled={busy} className="h-10 sm:h-8">{iPending ? "…" : "Ignore"}</Button></form>
      </div>
      <Result state={state} />
    </div>
  );
  ...
  return (
    <form action={action} className="grid gap-3 rounded-lg border border-line bg-zinc-50/50 p-3 sm:grid-cols-3">
      ...
      <div className="flex justify-end gap-2 sm:col-span-3">
        <Button type="button" size="sm" variant="ghost" onClick={() => setOpen(false)} className="h-10 sm:h-8">Cancel</Button>
        <Button size="sm" variant="primary" disabled={pending} className="h-10 flex-1 sm:h-8 sm:flex-none">{pending ? "Creating…" : "Create enquiry"}</Button>
      </div>
    </form>
  );
}
