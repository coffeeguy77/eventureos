    ...
    <div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_300px]">
        <Card className="min-w-0 overflow-hidden">
          {/* Toolbar */}
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-line px-3 py-3 sm:gap-3 sm:px-4">
            <div className="flex min-w-0 flex-1 flex-wrap items-center gap-x-2 gap-y-1 sm:flex-none">
              <ButtonLink href={href({ date: today })} size="sm" className="h-10 sm:h-8" aria-current={days.includes(today) ? "date" : undefined}>Today</ButtonLink>
              <div className="flex">
            ...
            <ul className="mt-2 space-y-1.5 text-[12px] text-ink-muted">
              {resources.map((r) => (
                <li key={r.id} className="flex items-center gap-2" title={r.externalCalendarId ? `Google calendar ID: ${r.externalCalendarId}` : "Not linked to an external calendar"}>
                  ...
                  <span className="shrink-0 text-ink-faint">{r.provider === "google" ? (r.syncEnabled ? "Google · sync on" : "Google · sync off") : "Local"}</span>
                </li>
              ))}
