...
export function AgendaView({ days, today, entries, resources, onOpen }: {
  ...
  return (
    <div>
      {groups.map(({ d, items }) => (
        <section key={d}>
          <h3 className={cn("sticky top-0 z-10 flex items-baseline gap-2 border-b border-line bg-zinc-50/95 px-5 py-2 text-[12px] font-semibold backdrop-blur",
              ...
              return (
                <li key={entry.id}>
                  <button type="button" onClick={() => onOpen(entry.id)}
                    className={cn("flex w-full items-start gap-3 px-5 py-2.5 text-left hover:bg-zinc-50/80", conflict && "bg-rose-50/70 hover:bg-rose-50")}>
                    <span className="tabular w-[92px] shrink-0 pt-0.5 text-[12.5px] text-ink-muted">
                    ...
                    {conflict && (
                      <span className="flex shrink-0 items-center gap-1 rounded-full bg-rose-600 px-2 py-0.5 text-[11px] font-semibold text-white">
